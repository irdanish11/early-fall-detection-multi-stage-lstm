...
class TSSTG(object):
    ...
    def __init__(self,
        ...
        self.model = StreamSpatialTemporalGraph(in_channels=3,
                                                graph_args=self.graph_args,
                                                num_class=self.num_class,
                                                edge_importance_weighting=True)
        self.model.load_state_dict(
            torch.load(sk_weight_file, map_location=torch.device('cpu')))
        self.model.eval()
        self.activation = {}
        self.model.dense2.register_forward_hook(self.get_activation('dense2'))

        self.model_lstm = MS_LSTM(1024, 1024, 30, self.num_class)
    ...
    def predict(self, pts, act_fts_arr, image_size):
        ...
        pts[:, :, :2] = normalize_points_with_size(pts[:, :, :2], image_size[0],
                                                   image_size[1])
