...
def get_action_aware_model(model_action):
    inp = Input(shape=model_action.layers[18].input.shape[1:])
    ...
    weights = model_action.layers[19].get_weights()
...
class ActionFeatures():
    def __init__(self,
        ...
        self.model_action = vgg_action(self.num_class,
                                       input_shape=(224, 224, 3))
        self.model_action.load_weights(self.action_weight_file)

        self.model_context = vgg_context(self.num_class,
                                         input_shape=(224, 224, 3))
        self.model_context.load_weights(self.context_weight_file)

        self.context_aware = K.function([self.model_context.layers[0].input],
                                        [self.model_context.layers[22].output])
        self.context_conv = K.function([self.model_context.layers[0].input],
                                       [self.model_context.layers[17].output])
        self.cam_conv = K.function([self.model_action.layers[0].input],
                                   [self.model_action.layers[19].output])
        self.cam_fc = self.model_action.layers[-1].get_weights()
        # The following code generates graph disconnected error
        # self.action_aware = K.function([self.model_action.layers[18].input],
        #                                [self.model_action.layers[22].output])
        # The following code implement the functionality of above line
        self.action_aware = get_action_aware_model(self.model_action)
    ...
    def get_action_features(self, image):
        ...
        return np.array(self.action_aware([feat_inp]))[0]
