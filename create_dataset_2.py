...
for scene in scenes:
    if scene == 'Home':
        ...
        save_path = 'data/Home_new-set(labelXscrw)_pred.pkl'
    if scene == 'Coffee_room':
        csv_pose_file = 'data/Coffee_room_new-pose+score.csv'
        save_path = 'data/Coffee_room_new-set(labelXscrw)_pred.pkl'
    # Params.
    ...
    for vid in vid_list:
        ...
        for fs in frames_set:
            ...
            for i in range(xys.shape[0] - n_frames):
                ...
                labels_set = np.append(labels_set, lb[i:i+n_frames], axis=0)
